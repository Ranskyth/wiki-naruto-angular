...
export class HomeComponent {
  ...
  get filteredApi() {
    return this.api.filter(item => {
      const matchSearch = item.title.toLowerCase().includes(this.search.toLowerCase());
      const matchClan = this.selectedClan ? item.caracteristicas.cla === this.selectedClan : true;
